# -*- coding: utf-8 -*-
"""
Created on Wed Nov  6 22:23:51 2019
...
#--------------------------------------------------------------------------------------------------------------------------#
# 数据转换
#train_test_data['certBeginDt'] = pd.to_datetime(train_test_data["certValidBegin"] * 10000) - pd.offsets.DateOffset(years=70)/10000
#print ("time >>>", train_test_data['certBeginDt'])
#train_test_data = train_test_data.drop(['certValidBegin'], axis=1)
#train_test_data['certStopDt'] = pd.to_datetime(train_test_data["certValidStop"] * 10000) - pd.offsets.DateOffset(years=70)/10000
#train_test_data = train_test_data.drop(['certValidStop'], axis=1)

#--------------------------------------------------------------------------------------------------------------------------#
# 特征组合 
train_test_data["certStopDt"+"certBeginDt"] = train_test_data["certValidStop"] - train_test_data["certValidBegin"]
...
train_test_data["certBeginDt_bin"] = pd.cut(train_test_data["certValidBegin"],30,labels=False)
train_test_data = train_test_data.drop(['certValidBegin'], axis=1)
train_test_data["certStopDt_bin"] = pd.cut(train_test_data["certValidStop"],30,labels=False)
train_test_data = train_test_data.drop(['certValidStop'], axis=1)
...
for train_index, test_index in kf.split(X_train):
    ...
    if auc > max_auc:
        ...
        print (">>>>", res)
        csv_file = 'res_xgb_kfold_cross' + str(n_splits) + '_'  + str(auc) + '.csv'
        res.to_csv( csv_file ) 